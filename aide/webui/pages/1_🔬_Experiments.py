"""
Main Experiments Page for AIDE ML
This page contains the core functionality for running AIDE ML experiments.
"""
...
class ExperimentsPage:
    """
    ExperimentsPage encapsulates the experiment running logic for AIDE ML.
    """

    def __init__(self):
        """
        Initialize the ExperimentsPage with environment variables and session state.
        """
    ...
    def config_session_state(self):
        """
        Initialize session state variables if they don't exist.
        """
        state_defaults = {
            "openai_key": self.env_vars["openai_key"],
            "anthropic_key": self.env_vars["anthropic_key"],
            "gemini_key": self.env_vars["gemini_key"],
            "openrouter_key": self.env_vars["openrouter_key"],
            "backend": "openai",
            "model": "gpt-4o",
            "uploaded_files": [],
            "is_running": False,
    ...
    def run(self):
        """
        Run the main logic of the experiments page.
        """
        st.title("🔬 AIDE ML Experiments")
        st.markdown("Run machine learning experiments with AI assistance")
    ...
    def render_sidebar(self):
        """
        Render the sidebar with API key and backend settings.
        """
        with st.sidebar:
            ...
            backend = st.selectbox(
                "Backend",
                options=["openai", "anthropic", "claude_code", "gemini", "openrouter", "hybrid"],
                index=["openai", "anthropic", "claude_code", "gemini", "openrouter", "hybrid"].index(
            ...
            model_options = {
                ...
                "claude_code": ["claude-opus-4", "claude-3-5-sonnet-20241022"],
    ...
    def render_input_section(self, results_col):
        """
        Render the input section with file uploads and settings.

        Args:
            results_col (st.delta_generator.DeltaGenerator): The results column for outputs.
        """
        ...
        # Example data loader
        if st.button("🏠 Load House Prices Example"):
            example_files = self.load_example_files()
            st.session_state.uploaded_files = example_files
            st.success(f"Loaded {len(example_files)} example files")
            st.rerun()

        # Use either uploaded files or example files
        files = self.get_data_files(uploaded_files)
        ...
        if st.button(
            ...
            if files and goal_text and eval_text:
                self.run_aide(files, goal_text, eval_text, num_steps, results_col)
            else:
                st.error("Please provide data files, goal, and evaluation criteria")
    ...
    def get_data_files(self, uploaded_files):
        """
        Get data files from either uploads or session state.

        Args:
            uploaded_files: Files uploaded by the user.

        Returns:
            list: List of files to use for the experiment.
        """
        if uploaded_files:
    ...
    def run_aide(self, files, goal_text, eval_text, num_steps, results_col):
        ...
        try:
            ...
            # Pass backend and model to experiment
    ...
    @staticmethod
    def initialize_experiment(input_dir, goal_text, eval_text, backend=None, model=None):
        """
        ...
        # Add backend and model if specified
        if backend:
    ...
    def render_results(self, results):
        ...
        with tabs[4]:
            ...
            self.render_validation_plot(results, step=st.session_state.get("current_step", 0))
